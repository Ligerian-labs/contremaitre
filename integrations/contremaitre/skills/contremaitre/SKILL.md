...
1. Run `contremaitre ensure --json`. It reuses or deploys this workspace and waits for readiness. If configuration is missing, read [setup](references/setup.md). Do not launch interactive assisted init from another agent.
...
After source edits, run ensure and verify again. Preserve the environment for the user's review. Public sharing and data deletion require the user's request; local preview links work on the runtime's Mac. Respect requests limited to explanation or review without running applications.
