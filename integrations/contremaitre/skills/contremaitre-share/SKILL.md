...
On connector failure, run `contremaitre tunnel logs [SERVICE] --json`. It reads the latest connector's stderr without starting sharing. Results include the log path, whether it exists, up to 64 KiB of output, and a truncation flag. Read the file when more detail is needed. Capture diagnostics before retrying because the next connector attempt replaces that service's log.
